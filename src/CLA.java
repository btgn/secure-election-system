...
public class CLA
{
    ...
    private String CTFServerName = null;

    private RSAPublicKey publicKey = null;

    private RSAPrivateKey privateKey = null;

    private RSAPublicKey publicCAKey = null;
    ...
    public boolean start() 
    {
        try 
        {
            ...
            publicKey = (RSAPublicKey)stream.readObject();
            stream.close();
            ...
            privateKey = (RSAPrivateKey)stream.readObject();
            stream.close();
            ...
            publicKeyCert = new byte[certLength];
            certStream.read(publicKeyCert, 0, publicKeyCert.length);
            certStream.close();

            stream = new ObjectInputStream (new FileInputStream("CA.public"));
            publicCAKey = (RSAPublicKey)stream.readObject();
            stream.close();
    ...
    class CLAServerThread extends Thread
    {
        ...
        public void run()
        {
            try 
            {
                ...
                output.writeObject(publicKeyCert);
                
                System.out.println("~~~ Using private RSA key to decrypt key from Voter");
                m_pkCipher.init(Cipher.DECRYPT_MODE, privateKey);
        ...
        private boolean doCTFRegister(BigInteger voteValidation)
        {
            ...
            {
                System.out.println("\n### Sending data to CTF");
                Socket socket = new Socket(CTFServerName, CLA_TO_CTF_PORT);
                ...
                requestStream.writeObject(publicKey);
                requestStream.writeObject(new String("ADD"));
                
                requestStream.writeObject(publicKeyCert);
